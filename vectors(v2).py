...
v1dotv2 = np.vdot(v1,v2) # 1 rad = 180x/pi  deg
v1xv2 = np.cross(v1,v2)
v2xv1 = np.cross(v2,v1)
magv1 = np.linalg.norm(v1)
magv2 = np.linalg.norm(v2)
theta_rad = np.arccos((v1dotv2)/(magv1*magv2))
theta_deg = (180*theta_rad)/np.pi

print('-------------------------------------------------------')
print(f"v1 = {v1[0]}i + " + f"{v1[1]}j + " + f"{v1[2]}k, " + f"Magnitude = {magv1}")
...
print(f'\nAngle between the Vectors is: {theta_deg} degrees')
print('-------------------------------------------------------')
